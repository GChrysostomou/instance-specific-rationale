...
# class BertModelWrapperQA(nn.Module):
    
#     def __init__(self, model):
    
#         super(BertModelWrapperQA, self).__init__()

#         """
#         BERT model wrapper for QA
#         """
...
#             self.mag = mag(hidden_dim = self.model.config.hidden_size)

#             assert args["mag_layer"] < self.model.config.num_hidden_layers

#         else: self.mag = None

        
#     def forward(self, input_ids, attention_mask, token_type_ids, salient_scores = None):        
        
#         embeddings, _ = bert_embeddings(self.model.bert, input_ids, 
#                                             position_ids = None, 
#                                             token_type_ids = token_type_ids
#                                         )
...
#         encoder_outputs = bert_encoder(self.model.bert,
#                                 embeddings,
#                                 attention_mask=extended_attention_mask,
#                                 head_mask=head_mask,
#                                 output_attentions=self.model.config.output_attentions,
...
#         if self.model.config.use_return_dict:
...
#             results = [start_logits, end_logits, attentions]
