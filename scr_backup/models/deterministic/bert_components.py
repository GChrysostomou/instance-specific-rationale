...
def bert_embeddings(bert_model, 
    ...
    if token_type_ids is None:
        ...
        token_type_ids = torch.zeros(input_shape, dtype=torch.long, device=position_ids.device) + 0.0000000001

    # print('INSIDE bert_embeddings, = bert_model.embeddings.word_embeddings(input_ids) ', input_ids)
    ...
    def forward(self, input_ids, attention_mask, token_type_ids, 
        ...
        if add_noise == False:
            ...
            if importance_scores.sum() ==  0: 
                # print(' ++++++++++++++++ importance 全为0, 那就是baseline, 没有rationales, zeroout全部sequence !!!!!!!!!!!!11')
                pass
            else:
            #### adding noise here
                importance_scores[:,0] = 1e-4 
