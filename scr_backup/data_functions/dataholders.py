...
class BERT_HOLDER():
    ...
    def __init__(self, path = str, b_size = 8 , 
                for_rationale = False, 
                stage = "train", return_as_frames = False, interpolation=False):
        """
        ...
        if interpolation == True: 
            with open(f"{path}train.json", "r") as file: train = json.load(file)[100:149]
            with open(f"{path}dev.json", "r") as file: dev = json.load(file)[100:149]
            with open(f"{path}test.json", "r") as file: test = json.load(file)[100:149]
        else:
