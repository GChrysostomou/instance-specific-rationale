    ...
    for epoch in range(epochs):
        ...
        for batch in training:
            ...
            if args.inherently_faithful:
                ...
                # print(batch["input_ids"].squeeze(1).to(device))
                # print(batch["lengths"].to(device))
                # print(torch.tensor(batch["label"]).to(device))
                # print(batch["token_type_ids"].squeeze(1).to(device))
                # print(batch["special tokens"])
                batch = {
                    "input_ids" : batch["input_ids"].squeeze(1).to(device),
                    "lengths" : batch["lengths"].to(device),
